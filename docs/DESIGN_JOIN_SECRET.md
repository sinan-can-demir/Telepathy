...
**Status: proposal, not built. Items marked OPEN need a decision before implementation.**
...
| **Invite handle** | Short numeric (OPEN: 6 digits) that finds an open invite. | Unique among *open* invites only; released when the invite is consumed, expires, or is burned. |
| **Invite code** | The secret: random, generated with `secrets` (OPEN: 6 Crockford-base32 chars, 30 bits). | Stored **keyed-hashed**, never plaintext; single use; at most 3 wrong attempts *per invite*, then burned; expires (OPEN: 15 min). |

An invitee types `handle` + `code` (displayed as one string, e.g. `482913-7KQ2MX`). One invite admits **one** person; adding another person means issuing another invite. No standing "join by PIN" path exists once a chat has started, and a seat vacated by a leaver reopens only through a fresh invite.
...
2. Compare with `hmac.compare_digest`. On a wrong code, increment the invite's failed counter *inside the same locked transaction*; at 3, mark it burned.
3. On success, mark the invite consumed, create the participant, notify the roster.

Attempts are counted **per invite, regardless of source address**. This replaces the per-IP limiter entirely (#95): nothing depends on `X-Forwarded-For`, and nothing collapses under Tor's shared address. A coarse global throttle may remain for *load* protection, but no security property depends on it.

### Why the locking matters

If the attempt counter is read, compared, then incremented outside a lock, an attacker can fire many guesses in parallel before any increment lands and get far more than 3. The check-and-increment must be atomic. This needs a concurrency test (see below), not just a unit test.
...
| **6 base32 chars** | **30** | **~3 in a billion** | **~0.003%** |
...
2. **Invites.** `ChatInvite` model; `POST /chat/create-invite/<chat_id>/` (any member: OPEN 4); `create-chat` issues the first invite; join takes handle+code; per-invite atomic attempt cap; single use; expiry; generic errors; keyed-hashed code. UI: two-part input in `usermenu.html`, invite display with expiry and "new invite" in the waiting overlay. Removes `Chat.pin`, the per-IP limiter (#95) and `check-chat` (#96, which has no first-party caller). Bounds the resource at the source: handles exist only while an invite is open.
...
1. **Code shape.** *Default: 6 Crockford-base32 chars* (no wordlist to vendor or license). Alternative: 3 words, easier to read aloud over a phone call, at the cost of a bundled wordlist.
2. **Wrong attempts before burning.** *Default: 3.* Note honest typos spend the same budget; a check character in the code (Crockford's checksum symbol) lets the client reject typos before they reach the server.
3. **Invite lifetime.** *Default: 15 minutes.*
4. **Who may issue invites.** *Default: any current member*, with the issuance announced to the whole roster. Alternative: creator only, which is stricter but strands a group if the creator leaves.
5. **Handle length.** *Default: 6 digits.* Longer makes sweeping costlier and typing longer.
6. **Burn vs timed lockout.** *Default: permanent burn.* A timed lockout lets an attacker keep an invite locked forever with three guesses per window, so it doesn't remove the denial of service and adds state. Burning fails safe and is simpler.
...
- **Typing cost:** 12 characters instead of 4 digits. That is the price of moving from 13 bits to a secret worth having.
