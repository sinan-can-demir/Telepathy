...
- **`services.sweep_expired_messages(chat)`**: called lazily on every `get-messages` fetch, the same "clean up on the next relevant request" pattern as `chat/auth.py`'s `IDLE_TIMEOUT` reclaim. (An abandoned chat, where nobody fetches any more, is deleted whole by the `reaper` service instead; see #97.) For every message with a TTL that isn't yet tombstoned, it checks whether *every currently-active participant who was already in the chat when the message was sent* has a read receipt older than `ttl_seconds`. Two things about that "who's required" set:
...
- **Tombstoning** (`services._tombstone`): the message's `compute_chain_hash()` result is computed and persisted to `Message.tombstone_hash` *before* anything is touched. Then `encrypted_text`/`aes_nonce`/`aes_tag`/`mac` are set to null, all its `MessageReadReceipt` rows are deleted — they have no remaining purpose once the content they refer to is gone. `chat/chain.py`'s `compute_chain_hash` checks `tombstone_hash` first and returns it directly when set, rather than recomputing from the now-null fields — this is the single piece of state that keeps the whole chain verifiable through an expiry, and it's a deliberate, narrow exception to that function's own "nothing here is ever persisted" design for an ordinary message.
